<?php
  ...
  class Core {
    ...
    public function __construct(){
      ...
      if(empty($this->url)) {$this->currentMethod = 'index';}
      ...
      if(!empty($this->url[$s]) && file_exists($rootDir . 'controllers' . _DS . strtolower($this->url[$s]). '.php')){

        // If exists, set as controller
        $this->currentController = ucwords($this->url[$s]);
      ...
      if(!empty($this->url[$s])){
        ...
        if(method_exists($this->currentController, str_replace('-', '', $this->url[$s]))){

          $this->currentMethod = str_replace('-', '', $this->url[$s]);
