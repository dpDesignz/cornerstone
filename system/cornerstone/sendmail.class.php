  ...
  class SendMail {
    ...
    */
    public function sendPHPMail($replyToEmail = '', $from = ADMIN_EMAIL,$fromName = SITE_NAME,$to,$toName = '',$subject,$htmlMessage = '',$textMessage = '',$ccArray = '') {
      ...
      if(empty($subject)){$subject = "An Email from ".SITE_NAME;}
      if(empty($replyToEmail)) {$replyToEmail = $from;}
      if(empty($textMessage)) {$textMessage = "Sorry there was no plain text version of this email sent. Please contact ".$from." for more information.";}
