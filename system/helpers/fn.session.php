<?php
  ...
  ini_set('session.gc_maxlifetime', (int)get_option("session_expire"));
  ...
  ini_set('session.cookie_domain', str_replace('www', '', get_option('site_url')));
  ...
  if (isset($_SESSION['_cs']['LAST_ACTIVITY']) && (time() - $_SESSION['_cs']['LAST_ACTIVITY'] > (int)get_option("session_expire"))) {
  ...
  } else if ((time() - $_SESSION['_cs']['CREATED']) > (int)get_option("session_expire")) {
 ...
 */
function flashMsg($name, $message = '', $class = 'success') {
  ...
  if(!empty($message) && empty($_SESSION['_cs']['msg_' . $name])) {
    ...
    echo '<div class="csc-alert csc-alert--' . $class . '">' . $_SESSION['_cs']['msg_' . $name] . '</div>';
...
function userPageProtect($logout = FALSE) {
  ...
  if(empty($_SESSION['_cs']['user'])) {
    ...
    if(!empty($_COOKIE['_cs'])) {
      ...
      // Load userauth model
      $userAuth = $controller->loadModel('cornerstone/userauth', 'admin');
