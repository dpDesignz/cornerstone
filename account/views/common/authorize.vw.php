...
$pageMetaTitle = "Authorize | " . $data->site_name;
$pageMetaDescription = $data->site_name . " user authorization page.";
$pageMetaImage = get_site_url('img/cornerstone_framework_logo_white.png');
$pageMetaCanonical = get_site_url('account/authorize');
...
<main>
  <section id="cs--user-form" class="csc-card center animated animatedFadeInUp fadeInUp">
    <form action="<?php echo get_site_url('account/authorize'); ?>" method="POST" id="user-auth-form" class="csc-form">
...
<script>
  ...
  $(document).ready(function() {
    let validator = $("#user-auth-form").validate({
      rules: {
        token: {
          required: true,
          minlength: 6
